...
/*****
FIX This
probably, if genre is not set, show an error page saying as much
****/

$genre = isset($_GET['genre']) ? htmlspecialchars($_GET['genre'], ENT_QUOTES, 'UTF-8') : '1';
...
$pages_whole = floor( $total_number_of_movies / $number_of_movies_per_page ); // Source: https://stackoverflow.com/questions/5797625/how-to-divide-numbers-without-remainder-in-php (KingCrunch's answer.) Added: 03/06/2018.
...
$sql_query = <<< sql
SELECT title.id, title.title_data, title.year_, title.date_added, genre.genre_1, genre.genre_2, genre.genre_3, mpaa_rating.mpaa_rating_data, status_.status_data, image.filename
FROM title
INNER JOIN genre
...
WHERE
	( status_.status_data = 'Enabled' ) AND
	( genre.genre_1 = "$genre" ) OR
	( genre.genre_2 = "$genre" ) OR
	( genre.genre_3 = "$genre" )
